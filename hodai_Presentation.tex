...
\frame{
    ...
    \begin{block}{Contributions}
        \begin{itemize}
            \item Development of control and scheduling \textbf{co-design} framework %endless related work here
            \item \textbf{Reactive} scheduling (adaptation)
            \item \textbf{Composable} interface (based on automata theory)
            \item A \textbf{Kalman filter} based feedback for schedulers
        \end{itemize}
    \end{block}
    \begin{block}{I will show:}
        \begin{itemize}
            \item Extension of \textbf{RTComposer} and of \textbf{ArduPilotMega}
            \item Proof of concept with \textbf{simulation}
            \item Proof of concept with a \textbf{real-life case-study}
            \item A \textbf{bridge} between control and software engineering
        \end{itemize}
    \end{block}
}	
...
\frame{ %Example
  ...
  \begin{block}{}
      \begin{itemize}
          ...
          \item After \HH, estimation quality (\qq) is best, after \LL it may degrade
...
\frame{
    ...
    \framesubtitle{The scheduler is ``in the loop''}
    \begin{center}
            \scalebox{0.9}{
                \tikzArch
            }
...
\frame{
    ...
    \begin{center}
        \begin{columns}
            ...
            \end{column}
            \begin{column}{0.4\textwidth}
                \begin{block}{A set of methods ($T$)}
                    \tiny
                    class ComponentA \{\\
                    ~~~~void \HH () \{ ... \}\\
                    ~~~~void \LL () \{ ... \}\\
    ...
    \begin{block}{Scheduling \buchi Game}
        \begin{itemize}
            ...
            \item Scheduler alphabet is $\Sigma_{schd} = 2^T$ {(here $T=\{H, L\} $)}
            \item Environment alphabet is $\Sigma_{env} = \mathbb{R}^n$ (feedback variables)
            \item There is an transition for any \textbf{possible} environmental outcome 
            \item Any variable can be used as feedback 
...
\frame{
    ...
    \begin{block}{}
        \begin{itemize}
            \item Process \& actuation noises affect the \textit{a-priory} estimation 
            \item Measurement errors affect only the measurement
            \item Their difference is an indicator for the over-all errors
        \end{itemize}
    \end{block}
}
...
\frame{
    ...
    \begin{block}{ Attitude and position controller }
        \begin{itemize}
            \item The {\textbf{vision}} component estimates the position
            \item Position controller outputs a desired angle \\(proportional to acceleration)
            \item We used standard PID controllers
        \end{itemize}
    \end{block}
}
...
\frame{
    ...
    \begin{block}{ Limitation of Kalman filter  }
        \begin{itemize}
            \item The system is  not linear
            \item Noise distributions are unknown and unstable
            \item Kalman filter adds complexity in the code
        \end{itemize}
    \end{block}
    \begin{block}{ We propose an alternative that also filters in two steps: }
        \begin{enumerate}
            \item \textcolor{red}{Predicts} with a linearized model
            \item \textcolor{blue}{Updates} with the vision and other sensors
        \end{enumerate}
    \end{block}
}
\frame{
    ...
    \begin{block}{ Basic equations of motion on $x$ axis}
        \begin{itemize}
            \item \textbf{Position}: $\textcolor{red}{\bar{x}[k+1]} = x[k] + dt \cdot x[k]$
            \item \textbf{Velocity}: $\textcolor{red}{\bar{v}[k+1]} = v[k] + dt \cdot a[k]$
            \item \textbf{Acceleration}: $\textcolor{red}{\bar{a}[k+1]} = \Sigma F_x / m \approx roll \cdot g $
        \end{itemize}
    \end{block}
}
\frame{
    ...
    \begin{block}{ Measurement vector}
        \begin{itemize}
            ...
            \item \textbf{Velocity}: $ \frac{\partial x}{\partial t}$
...
\frame{
    ...
    \begin{block}{ }
        $ \X[k] = K \cdot \textcolor{red}{\bar{\X}[k]} + (1-K) \cdot C^{-1} \cdot \textcolor{blue}{y[k]} $
    \end{block}
    \begin{block}{ }
        $ x_r = K_r \cdot \textcolor{red}{\bar{x}[k]} + (1-K_r) \cdot \textcolor{blue}{y_r[k]} $\\
        $ x_v = K_v \cdot \textcolor{red}{\bar{v}[k]} + (1-K_v) \cdot \textcolor{blue}{y_v[k]} $\\
        $ x_a = \textcolor{red}{\bar{a}}$
    \end{block}
    \begin{block}{ Overall noise estimation }
        $ \tilde{y}_{k|k} = \textcolor{red}{\bar{x}[k]} - \textcolor{blue}{y_r[k]} $
    \end{block}
    \begin{itemize}
        \item $K$ matrix is depended on the vision mode
    \end{itemize}
}
...
\frame{
    ...
    \begin{center}
        \includegraphics[width=90mm]{setup}
    \end{center}
}
\frame{
...
\frame{
    ...
    \begin{tabular}{m{15em} |  m{4em} | m{5em} }
        ...
        Only Low & 2.1\% & 30.0\\
        \hline \hline
        $A_{x}$  ($T_x=10$) & \textbf{\colorbox{yellow}{16.6\%}} & \textbf{\colorbox{yellow}{10.9}}\\
        \hline
        ...
        \hline
        $A_{err}$ \newline ($T_l=10 , T_h=15$) & \textbf{\colorbox{yellow}{11.7\%}} & \textbf{\colorbox{yellow}{11.3}} \\
        \hline \hline
        $A_{comp1}$ \newline ($T_x=10$ , $T_l=10$ , $T_h=15$)  &\textbf{\colorbox{yellow}{ 8.8\%}} & \textbf{\colorbox{yellow}{12.9}} \\
        \hline
...
\frame{
    ...
    \begin{block}{Contributions}
        \begin{itemize}
            \item Control and scheduling \textbf{co-design} framework %endless related work here
            \item Improve CPU utilization with \textbf{Reactive} scheduling
    ...
    \begin{block}{I presented:}
        \begin{itemize}
            \item Extension of \textbf{RTComposer}
            \item Proof of concept with \textbf{simulation}
            \item Proof of concept with a \textbf{real-life case-study}
            \item A \textbf{bridge} between control and software engineering
        \end{itemize}
