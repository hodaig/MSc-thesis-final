...
\todoil{probably this section (or parts of it) should be in appendix}
...
In Section~\ref{sec:architecture} we suggest to use automata for describe the task specification for the scheduler, in this section we elaborate the technical details of the specification automata, \todo{not sure we want to talk about techniques (they can be ferers as related/future work)} techniques and tools that can be used to analyze the system, and in the last part of this section we show a working scheduler prototype that we implement to create to prove the concept.

\subsection{Formal Definition of Guarded (specification) Automaton}
...
Formally \textit{task specification automata} is defined, based on Non-deterministic Buchi Automata (NBA), as tuple $A= \left < Q,\Sigma,\Delta,Q_0,Acc \right > $.
...
%TODO - include psado-code here: 1. product all automatons 2. remove all ``bad''/ overflow transitions with too much tasks 2. simplify the automata 4. check schedualbility

\subsection{GOAL Tool}
\subsection{Example of Using the Scheduler Interface Prototype}
\todoil{tests with the real scheduler and the resulting scheduling}
