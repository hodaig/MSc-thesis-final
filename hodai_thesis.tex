...
\
%TODO - continue here!
\section{Experiment setup} % (From Paper)
\label{sec:Experiment setup}

\subsection{Hardware - Quad-rotor drone}
\label{sec:Experiment setup-HW}
% the hardware
In order to perform the experiments we used an of-the-shelf quadrotor shown in Figure~\ref{fig:hover_in_lab} \todo{add the image} with improved flight controlle: a \textit{raspberry-pi}, a single-board computer~\cite{raspberry}, with the \textit{navio2}~\cite{navio} shield.
