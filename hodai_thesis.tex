...
\begin{claim}
    Assuming $C_1,C_2, ... , C_m$ are a valid components then the composed component $C_s=C_1 \cdot C_2 \cdot .. \cdot C_m \cdot C_reasource $ is valid component and if there is a wining strategy for the scheduler $A_s$ then the system defined by $C_s$ is schedulable.
\end{claim}
\begin{proof}
    ...
...
\begin{thm}
    Assume two components $c_1$ and $c_2$ are correct, then, the composition $c = c_1 \cdot c_2$ is correct.
\end{thm}
...
\subsection{Example of Using the Scheduler Interface Prototype}
