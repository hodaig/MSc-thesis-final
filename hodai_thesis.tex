...
Many researches~\cite{Efraim17, Efraim2017, Shlomo, shakernia1999landing, khalil2005optical, ahrens2009vision, bills2011autonomous, blosch2010vision} and available products~\cite{APM, bristeau2011navigation} begin to integrate sensors that based on visual input, i.e. on-board camera, especially for Micro Aerial Vehicles (MAV), who are usually equipped with a compact and  computer with limited resources.
As vision-based sensors usually required expensive computations for extracting the needed information, the developers often need a high consideration of the resource management.
In order to integrate, for example, optic-flow sensor in multi-rotors controllers, engineers often preferred to add dedicated processor for simplify the architecture (e.g. PX4FLOW sensor for APM~\cite{APM}), or use better (and expensive) processor.
For education purposes, i.e. academic researches, even add dedicated remote computer for the expensive vision computations~\cite{Efraim17,Efraim2017}.
All these reinforce the need for flexible but predictable techniques for resource managements and scheduling.
We believe that lack of dedicated scheduling framework for co-design of real-time and non-real-time computations is the main reason that engineers avoid of including those expensive computations within the existing embedded processors.
Review of the available optic-flow sensors for APM software, we can see that the optic-flow sensors consist of very lite processor in comparison to the available UAV controllers for the APM software, this indicates that the controllers original processor have enough processing power to perform all the tasks but they still prefer to separate them.
...
This paper proposes a scheduling method that implement time variable step sizes instead of a fixed sampling frequency. The switching times are incorporated into an optimization problem, thus resulting in an adaptive step-size control process. The use of variable time steps is shown to require less tuning and reduces the number of expensive model evaluations.
...
Zilberstein and Russell~\cite{Shlomo} made an distinction between \textit{contract}-based algorithms, and \textit{interruptible} algorithms.
...
The use of vision based sensors with different CPU consumption Vs. sensing accuracy trade-offs is highly influenced by the \textit{contract-based} controller interface presented in~\cite{pant2015co}.
...
The scheduling for resources vs. quality trade-offs was also researched from the communication resources aspects. The paper~\cite{wu2013event} consider a sensor data scheduling for remote state estimation, with constrained communication energy and bandwidth.
