...
\todo{fix title}
\subsection{A Component}

\begin{dfn}
    A component is a pair $\langle T,G\rangle$ where $T$ is a set of subroutines (representing, e.g., a class in C++) and $G=\langle A,\langle P_{schd}, P_{env}\rangle \rangle$ is \todo{Add an appendix on games} a generalized B{\"u}chi game (GNBG) over a generalized B{\"u}chi automaton $A= \langle Q,\Sigma,\Delta,q_0,\mathcal{F} \rangle $, such that:
    \begin{enumerate}
...
%TODO - we present real life experiments
