...
%TODO - maybe add paragraph with details and examples in the real word
%TODO - we can show diferent prespectives: 1. offline scheduling using only $P$, 2. online scheduling using $\tilde{y_k|k}$ with the explenation that in the real world the errors are not always normal.
...
%TODO - not realy like the actuators here
...
as show in Figure~\ref{fig:V_v-geomerty} $x = l * \sin (V_d)$ were $l$ (distance from window) can be calculated from $sz$. I our case because $V_d$ is usually relatively small we approximate it by $x = l * V_d$.

%TODO - add fig:V_v-geomerty
\begin{figure} %[htbp]
...
%TODO check if some of this need to be moved to Section~\ref{sec:kalman}
%TODO add section on the system linearized model
