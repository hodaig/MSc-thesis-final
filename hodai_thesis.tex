...
\todo{I dont know if this section is too much (is mostly taken from the proposal)}
...
\begin{dfn}
    A component is a set, $T$, of subroutines (representing, e.g., a class in C++) with a \todo{Add an appendix on games}generalized B{\"u}chi game (GNBG), $G=\langle A,\langle P_{schd}, P_{env}\rangle \rangle$, over a generalized B{\"u}chi automaton $A= \langle Q,\Sigma,\Delta,q_0,\mathcal{F} \rangle $, such that:
    \begin{enumerate}
...
The idea is that each component is designed to handle a specific task in the system. All its subroutines serve this task. The scheduling game, $G$, of the component specifies the interaction between the subroutines and the environment. The acceptance condition encodes the component requirements.
...
The scheduler feedback variable $y$ is the estimated position, in this example the component requirement is to ensure that $y$ never exceeds one (in position $y=1$ there is a cliff). \todo{this example is too early because the reader doesn't know the terms yet}
...
\begin{dfn}
    The composition of the components $C_1=\langle A_1, \langle P_{schd}^1, P_{env}^1\rangle\rangle$ and $C_2=\langle A_2, \langle P_{schd}^2, P_{env}^2\rangle\rangle$ is the component $C_c = C_1 \times C_2$ defined as follows:
    The subroutines set is $T_c=T_1 \cup T_2$.
    The schedule game is $G_c = G_1 \times G_2 = \langle A_1 \times A_2 ,\langle P_{schd}^1 \times P_{schd}^2, P_{env}^1 \times P_{env}^2 \rangle \rangle$, that is, the automaton $A_c= \langle Q_1 \times Q_2,\Sigma_c,\Delta_c,Q_0^1 \times Q_0^2,Acc_1 \cup Acc_2 \rangle$ is a product of $C_1$ and $C_2$ automata such that:
    \begin{enumerate}
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        \Procedure{S chedule}{$A_s$} \Comment{schedule the system with strategy automaton $A_s$ }
...
\begin{dfn}
    A component $C_i$ is ``complete'' if for each $t_1, \dots, t_m \subseteq T_i$, $\vec{y}_1, \dots , \vec{y}_m \in \R^n$ and $q_e \in P_{env}$ such that there is a path from $q_0$ to $q_e$ with the word $w= \vec{y}_1 , t_1 , \vec{y}_2 , t_2, \dots , \vec{y}_{m}, t_m$ we have that for each $\vec{y}_{m+1}$ that can be observed after any run of the system that corresponds to this word (i.e., after each observation $\vec{y}_j$ we schedule $t_j$) there is a transition from $q_e$ labeled with $\vec{y}_{m+1}$.
\end{dfn}

\begin{dfn}
    \todo{maybe stability requirements}
    A complete component $C_i$ is valid if in  any execution whose observations and task schedules constitute a word $\omega \in \mathcal{L}(A_i)$ the component achieves its control objectives.
\end{dfn}
...
\begin{claim}
    Assuming $C_1,C_2, ... , C_m$ are a valid components then the composed component $C_s=C_1 \cdot C_2 \times \cdots  \times C_m \cdot C_ {reasource}$ is valid component and if there is a wining strategy for the scheduler $A_s$ then the system defined by $C_s$ is schedulable system.
\end{claim}
\begin{proof}
...
%TODO - continue here!
\commentOut{
\begin{dfn}{Correctness of a Component:}
    A component $c$ is \todo{don't know how to call it} correct / ``schedulable'' if and only, for each infinite possible sequence of scheduler feedback variables in the system $\vec{Y} = \left \{ \vec{y}_1, \vec{y}_2 , ... \right \}$ ($\vec{y}_i \in \R^n$) there is a word $\omega \in L_{\omega}(A_c)$ such that $\Pi_2(\omega) = \vec{Y}$~\footnote{$\Pi_i(\{ \langle \sigma_{1,1} , \sigma_{1,2} \rangle , \langle \sigma_{2,1} , \sigma_{2,2} \rangle , ... \}) = \{ \sigma_{1,i} , \sigma_{2,i} , ... \}$}, and for each such $\omega$ a schedule according to $\Pi_1(\omega)$ satisfy the component requirements assuming the scheduler feedback variables observation is $\vec{Y}$.
\end{dfn}

Each ``Guarded automaton'' practically define a scheduling strategy that satisfy the requirements of the component.
Each infinite word $\omega \in \Sigma^\omega$ of the automata define a possible instance of task scheduling (one execution of the system) where in the time slot (iteration) $i$ the scheduler observation before iteration $i$ was $\Pi_2(\omega)[i]$ and the scheduler execute all the subroutines in $\Pi_1(\omega)[i]$.



\begin{dfn}{Guarded Automaton}
    is defined as non-deterministic B{\"u}chi automaton (NBA) $A= \left < Q,\Sigma,\Delta,Q_0,Acc \right > $.
    Where the \textit{alphabet} is $\Sigma = 2^T \times C$.
    Each such automaton define strategy that satisfy the requirements as follows:
    Each infinite word $\alpha \in \Sigma^\omega$ of the automata define an instance of a task scheduling (one execution of the system) where in the time slot (iteration) $i$ is define by the letter $\sigma= \langle T_i , C_i \rangle \in \Sigma$
    
    , and the automata language define all the possible scheduling.
...
\todo{the examples before consist of automata with the task on the nodes, need to say something about the difference }
...
%Gather all guarded automata of all the system tasks result in a ``composed automaton'' that eventually used to schedule the system tasks.
%Each infinite path on the composed automata (not necessarily with infinitely many mode transitions) represents a schedule that satisfy all the tasks requirements. 

%TODO - define $s(\omega_j)$ as the schedule of $\omega_j$
Definitions:
\begin{enumerate}
